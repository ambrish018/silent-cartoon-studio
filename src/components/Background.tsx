...
const Park: React.FC<{ frame: number }> = ({ frame }) => {
...
const Kitchen: React.FC = () => (
  <g>
    ...
    {/* Clouds in window */}
    <Cloud x={390} y={250} s={0.28} op={0.65} />
    <Cloud x={490} y={330} s={0.22} op={0.55} />
    {/* Shelf rail */}
    ...
    {[20, 65, 120, 185, 250].map((gy, i) => (
      <line key={i} x1={0} y1={1378 + gy} x2={1080} y2={1378 + gy + 12} stroke="#A06830" strokeWidth={2} opacity={0.25} />
    ))}
    {/* Floor */}
...
const Night: React.FC<{ frame: number }> = ({ frame }) => (
  <g>
    <defs>
...
const Plain: React.FC<{ frame: number }> = ({ frame }) => {
  const c1x = (frame * 0.12) % 1600 - 320;
  return (
    <g>
      <defs>
...
export const Background: React.FC<{ theme: Theme }> = ({ theme }) => {
  ...
  return (
    <AbsoluteFill>
      ...
      >
        {theme === "park"      && <Park      frame={frame} />}
        {theme === "kitchen"   && <Kitchen   />}
        {theme === "classroom" && <Classroom />}
        {theme === "night"     && <Night     frame={frame} />}
        {theme === "plain"     && <Plain     frame={frame} />}
      </svg>
    </AbsoluteFill>
  );
};
